...
def _do_plot_candle_html(date, p_open, high, low, close, symbol, save):
    ...
    p.segment(pd.to_datetime(date), high, pd.to_datetime(date), low, color="black")
    # noinspection PyUnresolvedReferences
    p.rect(pd.to_datetime(date)[inc], mids[inc], w, spans[inc], fill_color=__colorup__, line_color=__colorup__)
    # noinspection PyUnresolvedReferences
    p.rect(pd.to_datetime(date)[dec], mids[dec], w, spans[dec], fill_color=__colordown__, line_color=__colordown__)
